...
/* 3. Ensure all text and internal elements are readable */
body, .stApp, .stText, .stMarkdown, .st-bh, .st-bb {{
    font-size: 18px !important; 
    color: #FFFFFF !important;
}}

/* 4. Style the main title using HTML for centering and color */
h1 {{
    text-align: center;
...
/* 5. Style dimension headers (H2) and report headers (H2) to be Gold */
h2 {{
    color: {GOLD_COLOR} !important; 
    font-size: 32px;
}}

/* 6. Style sub-headers (H3) and report section headers (H3) to be Gold */
h3 {{
    color: {GOLD_COLOR} !important; 
    font-size: 24px;
}}

/* 7. Target standard Streamlit paragraph text and enforce size */
...
/* 8. Ensure text inside radio buttons is readable */
...
if api_key:
    ...
    if submitted:
        ...
        with st.spinner("Analyzing your answers..."):
            try:
                answers_text = "\n".join([f"- {key}: scored {RATING_SCALE[value]}" for key, value in user_answers.items()])
...
"""
                ...
                * **For Guidance:** [1-on-1 Coaching](https://www.ChangeYourFuture.net)
                """)
